...
All ten migrations (`06b17a0f30ad_create_domain_tables.py` for P2,
...
and creates `document_chunks` - none of these drop or touch any prior
table. Verify the full chain locally:

```bash
docker compose exec backend alembic upgrade head
docker compose exec backend alembic current          # -> ad3f998eece8 (head)
docker compose exec backend alembic downgrade -1      # drops document_chunks + the vector extension only
...
10. RAG - **in progress (P10A: provider-agnostic embedding foundation,
    ...
    `app/rag/chunking.py`), awaiting CTO local verification**
11. Product Factory
