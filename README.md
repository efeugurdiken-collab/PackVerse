...
All eleven migrations (`06b17a0f30ad_create_domain_tables.py` for P2,
...
`embedding_model`, and `embedding_provider` to `document_chunks` - none
of these drop or touch any prior table. Verify the full chain locally:

```bash
docker compose exec backend alembic upgrade head
docker compose exec backend alembic current          # -> e4ba9bdd172a (head)
docker compose exec backend alembic downgrade -1      # drops the P10B2 embedding columns only
...
## Ingestion (Sprint P10B2: RAG track, roadmap item 10)
...
**No HTTP endpoint this sprint** - `ingest_asset()` is a plain async
service function, callable directly (or from a script/REPL against a
running stack) and wireable to an API route or a worker job in a later
sprint, matching the pattern documented above under Job Queue & Worker
for background work that doesn't need one yet.
...
upfront existence check is only a fast-path that avoids a wasted
...
retrieval/similarity-search API, no chunk deletion/update endpoints, no
worker/background wiring - all deferred to a later sprint per the
RAG roadmap item.
...
10. RAG - **in progress (P10A: provider-agnostic embedding foundation,
    ...
    `document_chunks` rows - no HTTP endpoint, retrieval, or OCR yet),
    awaiting CTO local verification**
11. Product Factory
