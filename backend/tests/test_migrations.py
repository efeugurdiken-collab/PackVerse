...
P6_REVISION = "a1c8f7d2b3e9"  # head as of Sprint P6
...
def test_migration_revision_identifiers_match_expected() -> None:
    ...
    P2/P3/P4/P5/P6 reports cite these exact revision ids as the schema
    history, and each sprint's downgrade-to-previous test depends on the
    down_revision chain staying intact."""
    versions_dir = BACKEND_ROOT / "alembic" / "versions"
    for revision in (P2_REVISION, P3_REVISION, P4_REVISION, P5_REVISION, P6_REVISION):
...
def test_migration_downgrade_to_p2_preserves_domain_tables(test_sync_database_url: str) -> None:
    """P3's users, P5's llm_requests, and P6's agent_runs tables must all
    drop on downgrade to P2 while every P2 domain table survives
    untouched - this is what 'downgrade cleanly back to P2' means
    operationally, not just 'the command exits 0'. None of the three
    ...
    try:
        ...
        assert EXPECTED_TABLES - {"users", "llm_requests", "agent_runs"} <= table_names
    finally:
        command.downgrade(cfg, "base")
...
def test_migration_downgrade_to_p4_removes_only_p4_columns(
    ...
    """Downgrading from head (P6) to the explicit P4_REVISION target
    stops right after P4's own migration - P4 stays fully applied, only
    Sprint P5's (llm_requests) and P6's (agent_runs) additions are
    undone. So the P4 columns on assets, and the P3 users table, must
    all still be present; llm_requests and agent_runs must both be gone.
    Uses the explicit P4_REVISION target (not "-1") since head no longer
    *is* P4 - see test_migration_downgrade_one_step_from_head_removes_
    only_agent_runs_table for the "-1 from current head" case."""
    cfg = _alembic_config(test_sync_database_url)
    command.upgrade(cfg, "head")
    try:
        ...
        # present; P5's llm_requests and P6's agent_runs must not.
        assert EXPECTED_TABLES - {"llm_requests", "agent_runs"} <= table_names
        assert "llm_requests" not in table_names
        assert "agent_runs" not in table_names
...
def test_migration_downgrade_to_p5_removes_only_agent_runs_table(
    test_sync_database_url: str,
) -> None:
    """Downgrading from head (P6) to the explicit P5_REVISION target
    stops right after P5's own migration - P5 stays fully applied
    (llm_requests present), only Sprint P6's addition (agent_runs) is
    ...
    try:
        ...
        assert "agent_runs" not in table_names
        assert EXPECTED_TABLES - {"agent_runs"} <= table_names
...
def test_migration_downgrade_one_step_from_head_removes_only_agent_runs_table(
    test_sync_database_url: str,
) -> None:
    """`alembic downgrade -1` from head (P6) must remove exactly the
    agent_runs table and nothing else - every P1-P5 table, including
